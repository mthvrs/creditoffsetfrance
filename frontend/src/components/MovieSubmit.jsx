...
function MovieSubmit({ movie, onSuccess }) {
  ...
  const updatePostCreditScene = (index, field, value) => {
    ...
    // Don't sanitize on onChange - let user type freely, including spaces
    // Sanitization happens on submit only
  ...
  const handleSubmit = async (e) => {
    ...
    // CRITICAL FIX: Trim timecodes before validation
  ...
  const handleConfirmedSubmit = async () => {
    try {
      ...
      const sanitizedData = {
        ...
        ffec: formData.ffec.trim(),  // CRITICAL FIX: Trim before sending
        ffmc: formData.ffmc.trim(),  // CRITICAL FIX: Trim before sending
